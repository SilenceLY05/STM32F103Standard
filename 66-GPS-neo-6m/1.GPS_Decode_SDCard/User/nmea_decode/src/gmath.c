...
/* 计算两点之间的距离 返回以米为单位的距离 */
double nmea_distance(
        const nmeaPOS *from_pos,    /**< 从以弧度为单位的位置 */
        const nmeaPOS *to_pos       /**< 以弧度定位 */
        )
{
...
/* 计算两点之间的距离 使用一个扁球体地球模型的算法。返回以米为单位的距离 */
double nmea_distance_ellipsoid(
        const nmeaPOS *from_pos,    /**< 从以弧度为单位的位置 */
        const nmeaPOS *to_pos,      /**< 以弧度定位 */
        double *from_azimuth,       /**<(0)以弧度为单位的“从”方位 */
        double *to_azimuth          /**< (0)以弧度为单位的“to”方位 */
        )
{
    /* 所有变量 */
...
/* 点位置的水平移动 */
int nmea_move_horz(
    const nmeaPOS *start_pos,   /**< 以弧度为单位的起始位置 */
    nmeaPOS *end_pos,           /**< 结果位置以弧度为单位 */
    double azimuth,             /**< 方位（度） [0, 359] */
    ...
    )
{
    ...
    distance /= NMEA_EARTHRADIUS_KM; /* 地球表面覆盖的角距离 */
...
/* 简短的水平移动点的位置 这个函数使用一个扁球体地球模型的算法。 */
int nmea_move_horz_ellipsoid(
    const nmeaPOS *start_pos,   /**< 以弧度为单位的起始位置 */
    nmeaPOS *end_pos,           /**< (O)以弧度为单位的结果位置 */
    double azimuth,             /**< 以弧度为单位的方位角 */
    double distance,            /**< 距离 (km) */
    double *end_azimuth         /**< (O)以弧度为单位的终点方位 */
    )
{
    /* Variables */
    double f, a, b, sqr_a, sqr_b;
