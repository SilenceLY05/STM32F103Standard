...
/* 计算包的CRC-16 CRC的寄存器值由rcr_init给出，这样可以不一次计算出所有的数据结果
（一帧图片由150Kb，内部不能一次放下这么大的数据，需要分段计算） 
*data：要计算的数据的数组。 length：数据到大小。rcr_init：初始值。计算CRC*/
...
/* 发送设置图像格式包给上位机 number：寄存器序号 val：寄存器值 */
void ack_read_wincc(uint16_t number,uint8_t val)
{
	...
	crc_16 = calc_crc_16((uint8_t *)&packet_head,sizeof(packet_head)-2,crc_16);  //分段计算crc_16的校验码，计算包头的
...
/* 发送设置图像格式包给上位机 addr设备地址 type图像格式 width图像宽度 height图像高度 */
void set_wince_format(uint8_t addr,uint8_t type,uint8_t width,uint8_t height)
{
	uint16_t crc_16 = 0xFFFF;
...
/* 发送图包数据给上位机 addr设备地址 0或1  width图像宽度 height图像高度 */
int write_rgb_wincc(uint8_t addr,uint16_t width,uint16_t height)
{
	...
	{
		receiving_process();//处理前面接收到的数据
		do{
			set_wince_format(addr,PIC_FORMAT_RGB565,width,height);   //发送设置图像格式指令
			flag++;
			if(flag>5)
				LED1_ON;
