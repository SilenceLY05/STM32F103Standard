...
/* FIFO_GPIO_Config  FIFO_GPIO配置 */
static void FIFO_GPIO_Config(void)
{
	GPIO_InitTypeDef GPIO_InitStructure;
	
	/* 开启时钟 */
	RCC_APB2PeriphClockCmd(OV7725_OE_GPIO_CLK | OV7725_WRST_GPIO_CLK |OV7725_RRST_GPIO_CLK | OV7725_RCLK_GPIO_CLK | OV7725_WE_GPIO_CLK | OV7725_DATA_GPIO_CLK , ENABLE);
	...
	/* FIFO_DATA FIFO输出数据 */
	GPIO_InitStructure.GPIO_Pin = (OV7725_DATA_0_GPIO_PIN | OV7725_DATA_1_GPIO_PIN | OV7725_DATA_2_GPIO_PIN |
...
/* VSYNC_GPIO_Config  OV7725 VSYNC中断相关配置 */
static void VSYNC_GPIO_Config(void)
{
	...
	/* 初始化时钟，注意中断要开AFIO */
	RCC_APB2PeriphClockCmd(OV7725_VSYNC_GPIO_CLK | RCC_APB2Periph_AFIO,ENABLE);
...
/* OV7725_GPIO_Config 初始化控制摄像头和相接的IO */
...
/* Sensor_Init  Sensor初始化 返回1成功 返回0失败 */
...
/* 设置特殊效果 eff：特殊效果，参考范围【0~6】 0:正常  1:黑白  2:偏蓝  3:复古  4:偏红  5:偏绿  6:反相 */
...
void OV7725_Window_Set(uint16_t sx,uint16_t sy,uint16_t width,uint16_t height,uint8_t QVGA_VGA)
{
	...
	/***************HSTART*********************/
	/* 读取寄存器的原内容，HStart包含偏移值，在原始偏移值的基础上加上窗口偏移 */
	SCCB_ReadByte(&reg_raw,1,REG_HSTART);
	
	/* sx为窗口偏移，高8位存储在HSTART,低8位存储在HREF */
	...
	/***************VSTART*********************/
	/* 读取寄存器的原内容，VSTRT包含偏移值，在原始偏移值的基础上加上窗口偏移 */
	...
	/* 水平宽度，高8位存储在HSIZE，低1位存储在HREF */
	SCCB_WriteByte(REG_VSIZE,height>>1);
	
	/* 读取寄存器的原内容 */
	SCCB_ReadByte(&reg_raw,1,REG_HREF);
	
	/* 把水平宽度的低2位、垂直高度的低1位，水平偏移的低2位，垂直偏移的低1位配置添加到HREF */
...
/* 设置显示位置  sx：x起始显示位置 sy：y起始显示位置 
width：显示窗口宽度，要求跟OV7725_Windos_Set函数中的width一致 height：显示窗口高度，要求跟OV7725_Windos_Set函数中的height一致  */
