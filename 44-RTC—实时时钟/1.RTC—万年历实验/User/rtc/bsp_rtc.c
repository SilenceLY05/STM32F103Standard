...
/* RTC_CheckAndConfig检查并配置RTC  用于读取RTC时间的结构体指针 */
...
void RTC_Configuration(void)
{
  ...
  /* 等待 RTC 寄存器 同步 因为RTC时钟是低速的，内环时钟是高速的，所以要同步 */
  RTC_WaitForSynchro();
...
void Time_Regulate_Get(struct rtc_time *tm)
{
  ...
  printf("\r\n==============设置时间=====================================");
	...
	do{
		printf("\r\n 请输入年份(Please Set Years),范围[1970~2038]，请输入字符后回车");
		scanf("%d",&temp_num);
	...
	do{
		printf("\r\n 请输入月份(Please Set Months),范围[1~12]，请输入字符后回车");
		scanf("%d",&temp_num);
	...
	do{
		printf("\r\n 请输入日期(Please Set Dates),范围[1~%d]，请输入字符后回车",day_max);
		scanf("%d",&temp_num);
	...
	do{
		printf("\r\n 请输入时钟(Please Set Hours),范围[0~23]，请输入字符后回车");
		scanf("%d",&temp_num);
		if(temp_num <1 || temp_num > 23)
		{
			printf("\r\n您输入的数字是：%d，不符合要求",temp_num);
		}
		else
	...
	do{
		printf("\r\n 请输入分钟(Please Set Minutes),范围[0~59]，请输入字符后回车");
		scanf("%d",&temp_num);
		if(temp_num <1 || temp_num > 59)
		{
			printf("\r\n您输入的数字是：%d，不符合要求",temp_num);
		}
		else
	...
	do{
		printf("\r\n 请输入秒(Please Set Seconds),范围[0~59]，请输入字符后回车");
		scanf("%d",&temp_num);
		if(temp_num <1 || temp_num > 59)
		{
			printf("\r\n您输入的数字是：%d，不符合要求",temp_num);
...
/* Time_Display 显示当前时间值  TimeVar RTC计数值，单位为 s */
void Time_Display(uint32_t TimeVar,struct rtc_time *tm)
{
  static uint32_t FirstDisplay = 1;
	...
	//日期
	sprintf((char*)str,"Date = %d-%d-%d   ",tm->tm_year,tm->tm_mon,tm->tm_mday);
	...
	//时间
	sprintf((char*)str,"Time:  %0.2d:%0.2d:%0.2d  ",tm->tm_hour,tm->tm_min,tm->tm_sec);
