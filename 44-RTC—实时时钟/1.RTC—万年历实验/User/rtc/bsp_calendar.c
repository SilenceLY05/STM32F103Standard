...
uint8_t GetMoonDay(uint8_t month_p,unsigned short table_addr)
{
	switch (month_p)
	{
		...
		case 13:
			if((year_code[table_addr+2]&0x08) == 0) return(0);
				else            return(1);
...
uint8_t GetChinaCalendar(uint16_t year,uint8_t month,uint8_t day,uint8_t *p)
{
	...
	if(temp4>= temp3)
	{
		...
		flag_y = 0;
		if(GetMoonDay(month_p,table_addr) == 0)   temp1 = 29;//小月 29
		else    temp1 = 30;  // 大月30天
		//从数据表中取该年的闰月月份，如为0,则该年无闰月
...
/* GetChinaCalendarStr  输入公历日期得到农历字符串  str 存储农历日期字符串地址  15Byte */
...
/* GetJieQi 输入公历日期得到本月24节气日期，day<15返回上半月节气，反之返回下半月节气 */
