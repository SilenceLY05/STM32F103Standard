...
static void TIMx_GPIO_Config(void)
{
	...
	RCC_APB2PeriphClockCmd(GENERAL_MASTER_CLK | GENERAL_SLAVE1_CLK | GENERAL_SLAVE2_CLK,ENABLE);
	...
	GPIO_Init(GENERAL_SLAVE1_PORT,&GPIO_InitStructure);
	
	/* 配置TIM4 PB6为推挽复用输出 */
	GPIO_InitStructure.GPIO_Pin = GENERAL_SLAVE2_PIN;
	GPIO_Init(GENERAL_SLAVE2_PORT,&GPIO_InitStructure);
	
	GPIO_InitStructure.GPIO_Pin = GENERAL_MASTER_PIN;
	GPIO_Init(GENERAL_MASTER_PORT,&GPIO_InitStructure);
}
...
static void TIM_Mode_Config(void)
{
	...
	/* 使能TIM时钟 */
	RCC_APB1PeriphClockCmd(GENERAL_TIM_MASTER_CLK | GENERAL_TIM_SLAVE1_CLK | GENERAL_TIM_SLAVE2_CLK,ENABLE);
  ...
  --------------------------------------------------------------------------- */
	...
	TIM_TimeBaseStructure.TIM_Period = 4;
	TIM_TimeBaseInit(GENERAL_TIM_SLAVE2,&TIM_TimeBaseStructure);
	...
	TIM_OC1Init(GENERAL_TIM_SLAVE2,&TIM_OCInitStructure);   //初始化 TIM3 输出通道1
	...
	TIM_SelectSlaveMode(GENERAL_TIM_SLAVE2,TIM_SlaveMode_Gated);  //从模式配置：TIM4
	TIM_SelectInputTrigger(GENERAL_TIM_SLAVE2,TIM_TS_ITR2);       //设置输入触发源
	
	/* TIM 计数器溢出中断启动 */
	TIM_Cmd(GENERAL_TIM_SLAVE1,ENABLE);
	TIM_Cmd(GENERAL_TIM_SLAVE2,ENABLE);
