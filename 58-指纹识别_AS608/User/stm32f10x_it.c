...
void AS608_USART_IRQHandler(void)
{
	...
	if(USART_GetITStatus(AS608_USARTx,USART_IT_RXNE)!= RESET)
	{
		ucCh = USART_ReceiveData(AS608_USARTx);
	...
	if(USART_GetITStatus(AS608_USARTx,USART_IT_IDLE) == SET)
	{
		/* 写缓冲区完毕 */
		cbWriteFinish(&rx_queue);
		ucCh = USART_ReceiveData(AS608_USARTx);/*由软件序列清除中断标志位(先读USART_SR，然后读USART_DR)*/
	}
}
