...
void AS608_Config(void)
{
	...
	AS608_USART_GPIO_ClockCmd(AS608_USART_GPIO_CLK,ENABLE);
	AS608_USART_ClockCmd(AS608_USART_CLK,ENABLE);
	...
	/*完成串口的初始化配置*/
	USART_Init(AS608_USARTx,&USART_InitStructure);
	
	/*使能串口接收中断*/
	USART_ITConfig(AS608_USARTx,USART_IT_RXNE,ENABLE);
	USART_ITConfig(AS608_USARTx,USART_IT_IDLE,ENABLE);
	
	/*使能串口*/
	USART_Cmd(AS608_USARTx,ENABLE);
}

/* AS608_USART串口向指纹模块传输数据 */
static void AS608_SendData(uint8_t data)
{
	USART_SendData(AS608_USARTx,data);
	while(USART_GetFlagStatus(AS608_USARTx,USART_FLAG_TXE) == RESET);
}

/* AS608_USART串口向指纹模块传输包头信息 */
...
/* AS608_USART串口向指纹模块传输包标识 */
static void AS608_SendFlag(uint8_t flag)
{
	AS608_SendData(flag);
}

/* AS608_USART串口向指纹模块传输包长度 */
static void AS608_SendLength(uint16_t length)
{
	AS608_SendData(length>>8);
	AS608_SendData(length);
}

/* AS608_USART串口向指纹模块传输指令 */
static void AS608_SendCmd(uint8_t cmd)
{
	AS608_SendData(cmd);
}

/* AS608_USART串口向指纹模块传输校验和 */
static void AS608_SendCheck(uint16_t check)
{
...
uint16_t PS_GenChar(uint8_t BufferID)
{
	...
	AS608_DELAY_MS(600);        /*等待指纹识别模块处理数据*/
...
uint16_t PS_Connect(uint32_t *PS_Addr)
{
	...
	if(rx_data!=NULL)
	{
		...
		if(*(rx_data->head) == 0xEF && *(rx_data->head+1) == 0x01 && *(rx_data->head+6) == 0x07)  /*判断是不是模块返回的应答包*/
		{
			AS608_INFO("指纹模块的地址为：0x%x%x%x%x\r\n",*(rx_data->head+2),*(rx_data->head+3),*(rx_data->head+4),*(rx_data->head+5));
		}
...
uint16_t PS_DeletChar(uint16_t PageID,uint16_t N)
{
	...
	temp = 0x01+0x07+0x0C+(PageID>>8)+PageID+(N>>8)+(uint8_t)N;
	AS608_SendCheck(temp);
