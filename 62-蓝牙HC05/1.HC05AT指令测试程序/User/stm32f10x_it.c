...
void DEBUG_USART_IRQHandler(void)
{
	...
	if(USART_GetITStatus(DEBUG_USARTx,USART_IT_RXNE) != RESET)
	{
		...
		if(DEBUG_USART_ReceiveData.datanum < USART_BUFF_SIZE)
		{
			if((ucCh != 0x0a) && (ucCh != 0x0d))
			{
				DEBUG_USART_ReceiveData.usart_buff[DEBUG_USART_ReceiveData.datanum] = ucCh;//不接收换行回车
	...
	{
		DEBUG_USART_ReceiveData.receive_data_flag = 1;    //由软件序列清除中断标志位(先读USART_SR，然后读USART_DR)	
...
void BLT_USART_IRQHandler(void)
{
	...
	if(USART_GetITStatus(BLT_USARTx,USART_IT_IDLE) ==SET)
	{
