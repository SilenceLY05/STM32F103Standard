...
{
	static uint16_t pwm_index = 0;    //用于PWM计数
	...
	if(TIM_GetITStatus(BRE_TIMx,TIM_IT_Update) != RESET)
	{
		...
		else
		{
			...
			if(((rgb_color&0xFF0000)>>16) >= amplitude_cnt )
				BRE_TIMx->BRE_RED_CCRx = indexWave[pwm_index];   //根据PWM表修改定时器的比较值寄存器
			else
				BRE_TIMx->BRE_RED_CCRx = 0;   //根据PWM表修改定时器的比较值寄存器
			
			//绿
			if(((rgb_color&0x00FF00)>>8) >= amplitude_cnt )
				BRE_TIMx->BRE_GREEN_CCRx = indexWave[pwm_index];   //根据PWM表修改定时器的比较值寄存器
			else
				BRE_TIMx->BRE_GREEN_CCRx = 0;   //根据PWM表修改定时器的比较值寄存器
			
			//蓝
			if((rgb_color&0x0000FF) >= amplitude_cnt )
				BRE_TIMx->BRE_BLUE_CCRx = indexWave[pwm_index];   //根据PWM表修改定时器的比较值寄存器
			else
				BRE_TIMx->BRE_BLUE_CCRx = 0;   //根据PWM表修改定时器的比较值寄存器
		}
