...
/* 控制收发引脚 */
//进入接收模式，必须要有延时等待RS485处理完数据
#define RS485_RX_EN()      RS485_Delay(1000); GPIO_ResetBits(RS485_RE_GPIO_PORT,RS485_RE_GPIO_PIN); RS485_Delay(1000);

//进入发送模式，必须要有延时等待RS485处理完数据
#define RS485_TX_EN()      RS485_Delay(1000); GPIO_SetBits(RS485_RE_GPIO_PORT,RS485_RE_GPIO_PIN); RS485_Delay(1000);
...
#define RS485_DEBUG_FUNC()              do{\
																					if(RS485_DEBUG_FUNC_ON)\
																						printf("<<-RS485-DEBUG-FUNC->> Func:%s@Line:%d\n",__func__,__LINE__);\
																					}while(0)
