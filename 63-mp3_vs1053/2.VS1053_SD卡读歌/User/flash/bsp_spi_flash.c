...
DSTATUS TM_FATFS_FLASH_SPI_disk_ioctl(BYTE cmd,char *buff)
{
	FLASH_DEBUG_FUNC();
	...
	switch(cmd)
	{
		...
		case GET_SECTOR_SIZE:
			//flash最小写单元为页，256字节，此处取2页为一个读写单位
			*(WORD*)buff = 4096;
		break;
...
DSTATUS TM_FATFS_FLASH_SPI_disk_read(BYTE *buff,DWORD sector,UINT count)
{
	FLASH_DEBUG_FUNC();
	...
	//扇区偏移，外部Flash文件系统空间放在外部Flash后面6M空间
	sector += 512;
...
DSTATUS TM_FATFS_FLASH_SPI_disk_write(BYTE *buff,DWORD sector,UINT count)
{
	uint32_t write_addr;
	FLASH_DEBUG_FUNC();
	sector += 512;
