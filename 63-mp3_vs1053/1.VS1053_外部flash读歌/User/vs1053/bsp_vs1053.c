...
unsigned char SPI_ReadWrite(unsigned char writedata)
{
	...
	{
		if((SPITimeout--) == 0) return SPI_TIMEOUT_UserCallback(5);
	}
...
void VS1053_PlayerSong(uint8_t *filepath)
{
	...
	if(strstr((const char*)filepath,".flac") || strstr((const char*)filepath,".FLAC"))
		VS_LoadPatch((uint16_t *)vs1053b_patch,VS1053B_PATCHLEN);
	...
	if(result == 0)
	{
		...
		{
			i = 0;
			result = f_read(&file,buffer,BUFFSIZE,(UINT*)&bw);
