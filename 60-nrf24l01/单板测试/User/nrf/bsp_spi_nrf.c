...
/* SPI 的I/O配置 */
void SPI_NRF2_Config(void)
{
	GPIO_InitTypeDef     GPIO_InitStructure;
	SPI_InitTypeDef      SPI_InitStructure;
	
	/* 开启GPIO对应的时钟 */
	NRF2_GPIO_ClockCmd(NRF2_SCK_GPIO_CLK | NRF2_MOSI_GPIO_CLK | NRF2_MISO_GPIO_CLK | NRF2_CSN_GPIO_CLK | NRF2_CE_GPIO_CLK | NRF2_IRQ_GPIO_CLK ,ENABLE);
	
	/* 开启SPI1时钟 */
	NRF2_SPI_ClockCmd(NRF2_SPI_CLK,ENABLE);
	
	/* SCK,MISO,MOSI引脚 */
	GPIO_InitStructure.GPIO_Pin = NRF2_SCK_GPIO_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;    //复用推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_10MHz;
	GPIO_Init(NRF2_SCK_GPIO_PORT,&GPIO_InitStructure);
	
	GPIO_InitStructure.GPIO_Pin = NRF2_MISO_GPIO_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;    //复用推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_10MHz;
	GPIO_Init(NRF2_MISO_GPIO_PORT,&GPIO_InitStructure);
	
	GPIO_InitStructure.GPIO_Pin = NRF2_MOSI_GPIO_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;    //复用推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_10MHz;
	GPIO_Init(NRF2_MOSI_GPIO_PORT,&GPIO_InitStructure);
	
	/* 配置CSN CE IRQ 引脚 */
	GPIO_InitStructure.GPIO_Pin = NRF2_CSN_GPIO_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;   //推挽输出 
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_10MHz;
	GPIO_Init(NRF2_CSN_GPIO_PORT,&GPIO_InitStructure);
	
	GPIO_InitStructure.GPIO_Pin = NRF2_CE_GPIO_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;    
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_10MHz;
	GPIO_Init(NRF2_CE_GPIO_PORT,&GPIO_InitStructure);
	
	GPIO_InitStructure.GPIO_Pin = NRF2_IRQ_GPIO_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;      //上拉输入
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_10MHz;
  ...
  NRF2_CSN_HIGH(); 
	...
	SPI_Init(NRF2_SPIx,&SPI_InitStructure);
	
	SPI_Cmd(NRF2_SPIx,ENABLE);
	
}

/* 用于向NRF读写一个字节数据 */
uint8_t SPI_NRF2_ReadWrite(uint8_t data)
{
	/* 当SPI发送缓冲器非空时等待 */
	while(SPI_I2S_GetFlagStatus(NRF2_SPIx,SPI_I2S_FLAG_TXE) == RESET);
	
	/* 通过SPI1发送一个字节的数据 */
	SPI_I2S_SendData(NRF2_SPIx,data);
	
	/* 当SPI接收缓冲器为空时等待 */
	while(SPI_I2S_GetFlagStatus(NRF2_SPIx,SPI_I2S_FLAG_RXNE) == RESET);
	
	/* 返回从SPI总线独到的数据 */
	return SPI_I2S_ReceiveData(NRF2_SPIx);
}

/* 向寄存器reg写入一个字节，同时返回状态字节 */
uint8_t SPI_NRF2_WriteReg(uint8_t reg,uint8_t data)
{
	uint8_t status;
	...
	/* 发送命令及寄存器号 */
	status = SPI_NRF2_ReadWrite(reg);
	
	/* 向寄存器写入数据 */
	SPI_NRF2_ReadWrite(data);
...
uint8_t SPI_NRF2_ReadReg(uint8_t reg)
{
	...
	/* 发送寄存器号 */
	SPI_NRF2_ReadWrite(reg);
	
	/* 读取寄存器中的值 */
	reg_val = SPI_NRF2_ReadReg(NOP);
...
/* 用于向NRF寄存器中写入一串数据 reg:NRF命令+寄存器地址 pBuffer:用于存储将被写入的寄存器数据的数组，Num：pBuffer数据长度 返回寄存器状态 */
uint8_t SPI_NRF2_WriteBuffer(uint8_t reg,uint8_t *pBuffer,uint8_t Num)
{
	uint8_t status,byte_cnt;
	...
	/* 向缓冲区写入数据 */
	for(byte_cnt = 0;byte_cnt<Num;byte_cnt++)
		SPI_NRF2_ReadWrite(*pBuffer++);  //写数据到缓冲区 	
...
/* 用于从NRF寄存器中读出一串数据 reg:NRF命令+寄存器地址 pBuffer:用于存储将被读出的寄存器数据的数组，Num：pBuffer数据长度 返回寄存器状态 */
uint8_t SPI_NRF2_ReadBuffer(uint8_t reg,uint8_t *pBuffer,uint8_t Num)
{
	uint8_t status,byte_cnt;
	...
	/* 向缓冲区写入数据 */
	for(byte_cnt = 0;byte_cnt<Num;byte_cnt++)
		pBuffer[byte_cnt] = SPI_NRF2_ReadWrite(NOP);//从NRF24L01读取数据  
...
void NRF2_RX_Mode(void)
{
	...
	SPI_NRF2_WriteBuffer(NRF_WRITE_REG+RX_ADDR_P0,RX_ADDRESS,RX_ADR_WIDTH);   //写RX节点地址
	SPI_NRF2_WriteReg(NRF_WRITE_REG+EN_AA,0x01);  //使能通道0的自动应答
	SPI_NRF2_WriteReg(NRF_WRITE_REG+EN_RXADDR,0x01);   //使能通道0的接收地址
	SPI_NRF2_WriteReg(NRF_WRITE_REG+RF_CH,CHANNAL);    //设置RF通信频率
...
void NRF2_TX_Mode(void)
{
	...
	SPI_NRF2_WriteBuffer(NRF_WRITE_REG+TX_ADDR,TX_ADDRESS,TX_ADR_WIDTH);  //写TX节点地址 
	SPI_NRF2_WriteBuffer(NRF_WRITE_REG+RX_ADDR_P0,RX_ADDRESS,RX_ADR_WIDTH);   //设置TX节点地址，使能ACK
	SPI_NRF2_WriteReg(NRF_WRITE_REG+EN_AA,0x01);  //使能通道0的自动应答
	SPI_NRF2_WriteReg(NRF_WRITE_REG+EN_RXADDR,0x01);   //使能通道0的接收地址
	SPI_NRF2_WriteReg(NRF_WRITE_REG+SETUP_RETR,0x1a);  //设置自动重发间隔时间:500us + 86us;最大自动重发次数:10次
	SPI_NRF2_WriteReg(NRF_WRITE_REG+RF_CH,CHANNAL);    //设置RF通道为CHANAL
	SPI_NRF2_WriteReg(NRF_WRITE_REG+RF_SETUP,0x0f);     //设置TX发射参数，0db增益，2Mbps，低噪声增益开启
	SPI_NRF2_WriteReg(NRF_WRITE_REG+CONFIG,0x0e);       //配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,接收模式 
...
uint8_t NRF2_Check(void)
{
	...
	/* 写入5个字节的地址 */
	SPI_NRF2_WriteBuffer(NRF_WRITE_REG+TX_ADDR,buf,5);
	
	/* 读出写入的地址 */
	SPI_NRF2_ReadBuffer(TX_ADDR,buf2,5);
...
/* 用于向NRF的发送缓冲区中写入数据 txBuffer:存储了将要发送的数据的数组，外部定义  */
uint8_t NRF2_TX_Data(uint8_t *txBuffer)
{
	uint8_t status;
	...
	/* 写入数据到txBuffer最大32个字节 */
	SPI_NRF2_WriteBuffer(WR_TX_PLOAD,txBuffer,TX_PLOAD_WIDTH);
	...
	/* 读取寄存器的状态值 */
	status = SPI_NRF2_ReadReg(STATUS);
	
	/* 清除TX_DS或MAX_RT中断标志 */
	SPI_NRF2_WriteReg(NRF_WRITE_REG+STATUS,status);
...
/* 用于从NRF的接收缓冲区中读出数据 rxBuffer:用于接收该数据的数组，外部定义  */
uint8_t NRF2_RX_Data(uint8_t *rxBuffer)
{
	uint8_t status;
	...
	while(NRF2_Read_IRQ() == 0)
	{
		...
		if(status & RX_DR)           //达到最大重发次数
		{
			SPI_NRF2_ReadBuffer(RD_RX_PLOAD,rxBuffer,RX_PLOAD_WIDTH);    //读取数据
