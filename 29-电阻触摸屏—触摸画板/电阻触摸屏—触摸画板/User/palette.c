...
/* Touch_Button_Down 按键被按下时调用函数 有触摸屏调用 x触摸位置的x坐标 y触摸位置的y坐标 */
void Touch_Button_Down(uint16_t x,uint16_t y)
{
	...
	{
		/* 触摸到了按钮 */
		if(x<=button[i].end_x&&y<=button[i].end_y && x>=button[i].start_x && y>=button[i].start_y)
		{
...
/* Touch_Button_Up 按键被释放时调用函数 由触摸屏调用 x触摸最后释放时位置的x坐标 y触摸最后释放时位置的y坐标 */
void Touch_Button_Up(uint16_t x,uint16_t y)
{
	...
	for( i = 0 ; i<BUTTON_NUM;i++)
	{
		...
		if(x<button[i].end_x&&y<button[i].end_y && x>button[i].start_x && y>button[i].start_y)
		{
			button[i].touch_flag =0;  /* 原本的状态为没有按下，则更新状态 */
...
/* Draw_Trail在画板区域描绘触摸轨迹  pre_x上一点的x坐标 pre_y上一点的y坐标 x最新一点的x坐标 y最新一点的y坐标 */
	...
	if(x>PALETTE_START_X && pre_x>PALETTE_START_X)
	{
		...
		switch(brush->shape)  /*根据画刷参数描绘不同的轨迹*/
		{
			...
			case LINE_WITH_CIRCLE:
				...
				if(x-3<PALETTE_START_X || pre_x-3 < PALETTE_START_X)
					break;
...
/* Draw_Color_Button  颜色按钮的描绘函数 btn Touch_Button类型的按键参数*/
static void Draw_Color_Button(void *btn)
{
...
/* Draw_Clear_Button 清屏按钮的描绘函数 btn Touch_Button类型的按键参数 */
static void Draw_Clear_Button(void *btn)
{
	Touch_Button *ptr=(Touch_Button *)btn;
...
/* Draw_Shape_Button  笔刷按钮的描绘函数 btn Touch_Button类型的按键参数*/
static void Draw_Shape_Button(void *btn)
{
	Touch_Button *ptr=(Touch_Button *)btn;
	uint16_t i;
	...
	switch (ptr->para)
	{
		case LINE_SINGLE_PIXCEL:
			LCD_SetColors(CL_BLACK,CL_WHITE);
			ILI9341_DrawLine(ptr->start_x+20,ptr->start_y +(ptr->end_y - ptr->start_y)/2,
											 ptr->end_x-20,ptr->start_y +(ptr->end_y - ptr->start_y)/2);
		break;
		
		case LINE_2_PIXCEL:
			LCD_DrawUniLineCircle(ptr->start_x+20,ptr->start_y +(ptr->end_y - ptr->start_y)/2,
		...
		case LINE_20_PIXCEL:
			LCD_DrawUniLineCircle(ptr->start_x+20,ptr->start_y +(ptr->end_y - ptr->start_y)/2,
														ptr->end_x-20,ptr->start_y +(ptr->end_y - ptr->start_y)/2,10);

		break;
		
		case LINE_WITH_CIRCLE:
			LCD_SetColors(CL_BLACK,CL_WHITE);
			ILI9341_DrawLine(ptr->start_x+5,ptr->start_y +(ptr->end_y - ptr->start_y)/2,
		...
		case RUBBER:
			...
			LCD_SetFont(&Font8x16);
			ILI9341_DispString_EN_CH(ptr->start_x + (ptr->end_x - ptr->start_x - 16*2)/2 , 
															 ptr->start_y + ((ptr->end_y - ptr->start_y -24 -16)/2)+24,"橡皮");
...
/* Command_Select_Color  笔刷按钮的描绘函数 btn Touch_Button类型的按键参数*/
static void Command_Select_Color(void *btn)
{
	Touch_Button *ptr = (Touch_Button *)btn;
...
/* Command_Select_Brush  笔刷按钮的描绘函数 btn Touch_Button类型的按键参数*/
static void Command_Select_Brush(void *btn)
{
	Touch_Button *ptr = (Touch_Button *)btn;
...
/* Command_Clear_Palette  切换画刷颜色 清屏按键的公共执行函数 btn Touch_Button类型的按键参数*/
static void Command_Clear_Palette(void *btn)
{
