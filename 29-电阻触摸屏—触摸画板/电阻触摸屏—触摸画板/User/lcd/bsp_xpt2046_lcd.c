...
/* 对XPT2046选择一个模拟通道后，启动ADC 并返回ADC采样结果 */
static uint16_t XPT2046_ReadAdc( uint8_t ucChannel)
{
...
/* 读取XPT2046的X通道和Y通道的AD值（12bit，最大为4096） sX_Ad:存放X通道AD值的地址   sY_Ad:存放Y通道AD值的地址 */
static void XPT2046_ReadAdc_XY(int16_t * sX_Ad, int16_t * sY_Ad)
{
...
/* 在触摸XPT2046屏幕时获取一组坐标的AD值，并对改坐标进行滤波，返回滤波之后的AD值 */
...
static uint8_t XPT2046_ReadAdc_Smooth_XY( strType_XPT2046_Coordinate * pScreenCoordinate )
{
	...
	int16_t sAD_X,sAD_Y;
	int16_t sBufferArray [ 2 ] [ 9 ] = {{ 0 }, { 0 }};  //对坐标X和Y进行9次采样
	...
	}while( ( XPT2046_EXTI_Read() == XPT2046_EXTI_ActiveLevel ) && ( ucCount < 9 ) ); // 用户点击触摸屏时，即TP_INT_IN信号为低 并且 ucCount<9
	...
	if( ucCount == 9 )
	{
		...
		/* 判断绝对差值是否超过门限值，如果这3个绝对差值都超过门限值，则判定采样点不准确，抛弃采样点，差值门限取2 */
		...
		/* 计算他们的平均值，同时赋值为strScreenCoordinate */
		if( lDifference[ 0 ] < lDifference[ 1 ] )
		{
			if( lDifference[ 2 ] < lDifference[ 0 ] )
				pScreenCoodinate->x = ( lAverage[ 0 ] + lAverage[ 2 ] ) / 2;
			else
				pScreenCoodinate->x = ( lAverage[ 0 ] + lAverage[ 1 ] ) / 2;
		}
		else if (lDifference[ 2 ] < lDifference[ 1 ])
			pScreenCoodinate->x = ( lAverage[ 0 ] + lAverage[ 2 ] ) / 2;
		else
			pScreenCoodinate->x = ( lAverage[ 1 ] + lAverage[ 2 ] ) / 2;
		...
		{
			if( lDifference[ 2 ] < lDifference[ 0 ] )
				pScreenCoodinate->y = ( lAverage[ 0 ] + lAverage[ 2 ] )/2;
			else
				pScreenCoodinate->y = ( lAverage[ 0 ] + lAverage[ 1 ] )/2;
		}
		else if (lDifference[ 2 ] < lDifference[ 1 ])
			pScreenCoodinate->y = ( lAverage[ 0 ] + lAverage[ 2 ] )/2;
		else
			pScreenCoodinate->y = ( lAverage[ 1 ] + lAverage[ 2 ] )/2;
...
static uint8_t XPT2046_ReadAdc_Smooth_XY(strType_XPT2046_Coordinate* pScreenCoordinate)
{
	...
	if( ucCount == 10 )
	{
		...
		sBufferArray[0][5] +sBufferArray[0][6] +sBufferArray[0][7] +sBufferArray[0][8] +sBufferArray[0][9] - lX_Max-lX_Min)>>3;
		
		pScreenCoordinate->y = ( sBufferArray[1][0] + sBufferArray[1][1] +sBufferArray[1][2] +sBufferArray[1][3] +sBufferArray[1][4] +
		sBufferArray[1][5] +sBufferArray[1][6] +sBufferArray[1][7] +sBufferArray[1][8] +sBufferArray[1][9] - lY_Max-lY_Min)>>3;
...
/* 计算XPT2046触摸坐标校正系数（注意：只有在LCD和触摸屏间的误差角度非常小时，才能运用下面公式）
	 pDisplayCoordinate:屏幕人为显示的已知坐标
	 pstrScreenSampel:对已知坐标点触摸时 XPT2046产生的坐标
	 pCalibrationFactor:根据人为设定坐标和采样回来的坐标计算出来的屏幕触摸校正系统
   返回值为1表示成功0表示失败*/
static uint8_t XPT2046_Calculate_CalibrationFactor ( strType_XPT2046_Coordinate * pDisplayCoordinate, strType_XPT2046_Coordinate * pScreenSample, strType_XPT2046_Calibration * pCalibrationFactor )
{
	uint8_t ucRet = 1;
...
/* 在ILI9341上显示校正触摸是需要的十字 usX：在特定扫描方向下十字交叉点的X坐标 usY：在特定扫描方向下十字交叉点的Y坐标*/
static void ILI9341_DrawCross(uint16_t usX,uint16_t usY)
{
...
/* XPT2046触摸屏校准  LCD_Mode：指定要校正哪种液晶扫描模式的参数 */
	...
	else   //若标志存在且不强制校正，则直接从FLASH中读取
	{
		SPI_FLASH_BufferRead((uint8_t *)&strXPT2046_TouchPara,FLASH_TOUCH_PARA_ADDR,4*6*8);
			...
			#if 0  //输出调试信息，注意要先初始化串口
			{
				...
				float * ulHeadAddres ;
				/* 打印校校准系数 */ 
				XPT2046_INFO ( "从FLASH里读取得到的校准系数如下：");
...
/* 获取XPT2046触摸点的坐标 */
uint8_t XPT2046_Get_TouchedPoint(strType_XPT2046_Coordinate * pDisplayCoodinate,strType_XPT2046_TouchPara * pTouchPara )
{
	uint8_t ucRet = 1;
	...
	{
		pDisplayCoodinate->x = ( ( pTouchPara[LCD_SCAN_MODE].dX_X*strScreenCoordinate.x ) + ( pTouchPara[LCD_SCAN_MODE].dX_Y*strScreenCoordinate.y ) + pTouchPara[LCD_SCAN_MODE].dX);
		pDisplayCoodinate->y = ( ( pTouchPara[LCD_SCAN_MODE].dY_X*strScreenCoordinate.x ) + ( pTouchPara[LCD_SCAN_MODE].dY_Y*strScreenCoordinate.y ) + pTouchPara[LCD_SCAN_MODE].dY);
	}
...
/* 触摸检测状态机 返回值： TOUCH_PRESSED表示触摸按下  TOUCH_NOT_PRESSED表示无接触 */
	...
	switch(touch_state)
	{
		...
		case XPT2046_STATE_WAITING:
			if(XPT2046_PENIRQ_Read() == XPT2046_PENIRQ_ActiveLevel)
			{
				...
				//如在定时器中调用，每10ms调用一次，则消抖时间为：DURIATION_TIME *10
				if( i >DURIATION_TIME)
				{
...
/* 触摸屏被按下时会调用本函数 touch包含触摸坐标的结构体 在本函数中编写自己的触摸按下处理应用 */
...
/* 触摸屏释放的时候会调用本函数 touch包含触摸坐标的结构体 在本函数中编写自己的触摸按下处理应用 */
void XPT2046_TouchUp(strType_XPT2046_Coordinate * touch)
{
	...
	/*处理触摸画板的选择按钮*/
	Touch_Button_Up(touch->x,touch->y);
	
	/***在上面编写自己的触摸按下处理应用***/
}

/* 检测到触摸中断时调用的处理函数，通过它调用tp_down和tp_up汇报触摸点
	 本函数需要在while循环里被调用，也可以使用定时器定时调用
