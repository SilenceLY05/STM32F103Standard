...
/* 缩放字模，缩放后的字模1个像素点由8个数据位来表示，0x01表示笔迹0x00表示空白区
	 in_width:原始字符宽度
	 in_heig:原始字符高度
	 out_width:缩放后的字符宽度
	 out_heig:缩放后的字符高度
	 in_ptr:字库输入指针，注意：1pixel 1bit
	 out_ptr：缩放后的字符输出指针 out_ptr实际上没有正常输出，改成了直接输出到全局指针zoomBuff中
	 en_cn：0为英文1为中文*/
void ILI9341_zoomChar(uint16_t in_width,uint16_t in_heig,uint16_t out_width,
uint16_t out_heig,uint8_t *in_ptr,uint8_t *out_ptr,uint8_t en_cn)
{
	uint8_t *pts,*ots;
	//根据原字模及目标字模的大小，设定运算比例因子，左移16位是为了把浮点运算转成定点运算
	...
	if(in_width*in_heig >= 1024) return; //限制输入大于32*32
		
	if(out_width*out_heig == 0 ) return;
	if(out_width*out_heig >= ZOOMMAXBUFF) return;  //限制最大缩放128*128
	pts = (uint8_t *)&zoomTempBuff;
	
	//为方便运算，字库的数据由1pixel/1bit映射到1pixel/8bit
	//0x01表示笔迹0x00表示空白区
	if(en_cn == 0x00)//英文
	{
		//英文与中文字库上下边界不对，可在此处调整，需要注意tempBuff防止溢出
		for(byteCount = 0 ;byteCount<in_heig*in_width/8;byteCount++)
		{
			for(bitCount = 0;bitCount <8; bitCount++)
			{
				//把原字模数据由位映射到字节
				//in_ptr里bitX为1，则ptr里整个字节值1
				//in_ptr里bitX为0，则ptr里整个字节值0
...
/* 利用缩放后的字模显示字符  usX：字符显示位置X，usY：字符显示位置Y，
Font_Width字符宽度 Font_Height字符高度 c要显示的字符数据 DrawModel：是否反色显示 */
...
/* 利用缩放后的字模显示字符串  x：字符显示位置X，y：字符显示位置Y，
Font_Width字符宽度 Font_Height字符高度 c要显示的字符数据 DrawModel：是否反色显示 */
...
/* 利用缩放后的字模显示字符串(沿Y轴方向)  x：字符显示位置X，y：字符显示位置Y，
Font_Width字符宽度 Font_Height字符高度 c要显示的字符数据 DrawModel：是否反色显示 */
