...
/* 分析缓冲区并将结果放入信息结构 返回解析报文数 */
int nmea_parse(    
    nmeaPARSER *parser,               //nmeaPARSER 解码时使用的缓冲区
    const char *buff, int buff_sz,    //buff 将要解码的GPS原始数据 buff_sz是buff的大小
    nmeaINFO *info                    //存储解码结果
    )
{
    int ptype, nread = 0;
    void *pack = 0;
        ...
        else if(crc >= 0)
        {
            ptype = nmea_pack_type(
                ...
                parser->buff_use - nparsed - 1);
								...
								//malloc表示动态分配一个sizeof(nmeaParserNODE)大小的内存
...
/* 分析缓冲区并将结果保存到解析器中 返回值：从缓冲区解析的字节数 */
