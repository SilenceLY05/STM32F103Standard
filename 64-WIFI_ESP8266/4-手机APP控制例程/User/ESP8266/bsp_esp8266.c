...
{
	// cStr存储网络协议、IP和端口信息的字符串    cCmd存储完整的AT命令的字符串
...
/* 查询已接入设备的IP pStaIp,存放已接入设备的IP 返回1有接入设备 0无接入设备 */
uint8_t ESP8266_CWLIF(char *pStaIp)
{
	...
	if(pCh)
	{
		pCh1 = strstr(strEsp8266_Fram_Record.Data_RX_BUF,"AT+CELIF\r\r\n") +11;
		ucLen = pCh - pCh1;
...
/* 设置模块的AP IP pApIp 模块的AP IP返回1设置成功 0设置失败 */
uint8_t ESP8266_CIPAP(char *pApIp)
{
	char cCmd[30];
	
	sprintf(cCmd,"AT+CIPIP=\"%s\"",pApIp);
...
/* 设置模块的STA IP pStaIp 模块的AP IP返回1设置成功 0设置失败 */
