...
#define FWI_DEFAULT 4   //Ä¬ÈÏµÄFWI
                                      ...
                                      uint16_t *piRxN)
{
    //int            iRev  = 0;
    int32_t status;
    int32_t            iTxNum = 0; /*the length of transmitted datas*/
    int32_t            iTxLmt = 0; /*the maxium I-block length*/
    int32_t            iTxCn = 0; /*the last transmitted I block length*/
    //  int8_t  aucTxBuf[256];/*{PCB} + [NAD] + [CID] + [INF] + {CRC16}*/
    transceive_buffer  mf_com_data;
    uint8_t *aucTxBuf = mf_com_data.mf_data;
    //  int8_t   aucRxBuf[256];/*{PCB} + [NAD] + [CID] + [INF] + {CRC16}*/
    uint8_t* aucRxBuf = mf_com_data.mf_data;
    uint8_t* pucTxBuf  = aucTxBuf;
    uint8_t* pucRxBuf  = aucRxBuf;
    int32_t            iSRetry   = 0; /*continuous s block count*/
    int32_t            iIRetry   = 0; /*i block re-transmission count*/
    int32_t            iErrRetry = 0; /*error cause re-transmission count*/
    int32_t   uiWtx     = 1;/*the extra wait time in communication*/
    int32_t s_swt_limit_count; /*after the special counts(user control), pcd responds timeout,if the card always sends s-block
            ...
            case RCV_IBLOCK:
                ...
                else
                {
                       ...
                       (aucRxBuf[0] & ISO14443_CL_PROTOCOL_ISN))
                    {
                        ...
                        {
                            if(aucRxBuf[0] & ISO14443_CL_PROTOCOL_CID)
                            {
                                pucRxBuf++;    /*jump over CID*/
                            }
                            if(aucRxBuf[0] & ISO14443_CL_PROTOCOL_NAD)
                            {
                                pucRxBuf++;    /*jump over NAD*/
                            }
                            /*R(ACK)*/
                            eExStep = ORG_ACKBLOCK;
                        }
