...
uint16_t gausMaxFrameSizeTable[] =
{
...
//printf("MI_COLLERR befor status = 0x%2x\r\n",status);
    ...
    if(MI_OK == status)
    {
        ...
        else
        {
            if(!(ATS[1]&BIT7))
            {
                ...
                {
                    tb = 1;
                    gtPcdModuleInfo.uiFwi = (ATS[2+(uint8_t)ta] & 0xF0) >> 4;
                    gtPcdModuleInfo.uiSfgi = ATS[2+(uint8_t)ta] & 0x0f;
